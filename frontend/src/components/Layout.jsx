...
const Layout = ({ theme, toggleTheme, userRole, onLogout }) => {
    ...
    return (
        <div className="d-flex flex-column min-vh-100">
            <nav className="navbar navbar-expand-lg fixed-top" style={{ background: 'var(--nav-bg)', borderBottom: '1px solid var(--border-color)', padding: '1rem 0' }}>
                    ...
                    <div className="nav-links ms-auto d-flex align-items-center gap-3">
                        {userRole === 'admin' ? (
                            <>
                                ...
                                <Link to="/contact-us" className={`nav-link-custom ${activeTab === 'contact-us' ? 'active' : ''}`}>
                                    Contact
                                </Link>
                            </>
                        ) : (
                            <>
                                ...
                                <Link to="/my-sessions" className={`nav-link-custom ${activeTab === 'my-sessions' ? 'active' : ''}`}>
                                    Sessions
                                </Link>
                                <Link to="/chat" className={`nav-link-custom ${activeTab === 'chat' ? 'active' : ''}`}>
                                    Chat
                                </Link>
                                <Link to="/grid-points" className={`nav-link-custom ${activeTab === 'grid-points' ? 'active' : ''}`}>
                                    Wallet
                                </Link>
                        ...
                        <div className="vr mx-2" style={{ height: '20px', opacity: 0.2, background: 'var(--text-muted)' }}></div>
                        ...
                        {userRole === 'guest' ? (
                            <>
                                ...
                                <Link to="/contact-us" className="nav-link-custom d-none d-md-block">Contact</Link>
            ...
            <div className="flex-grow-1 w-100" style={{ paddingTop: '80px' }}>
                <Outlet />
            </div>
            <Footer />
        </div>
